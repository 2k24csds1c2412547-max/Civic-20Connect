...
export default function GamificationHub() {
  ...
  const leaderboard: LeaderboardEntry[] = [
    { rank: 1, name: "Sarah M.", points: 3250, reportsSubmitted: 67, badge: "🏆" },
    { rank: 2, name: "Mike K.", points: 2890, reportsSubmitted: 54, badge: "🥈" },
    { rank: 3, name: "Emma L.", points: 2640, reportsSubmitted: 48, badge: "🥉" },
    { rank: 4, name: "Alex R.", points: 2150, reportsSubmitted: 41, badge: "⭐" },
    { rank: 5, name: "Lisa T.", points: 1980, reportsSubmitted: 37, badge: "⭐" },
    { rank: 6, name: "Tom W.", points: 1750, reportsSubmitted: 32, badge: "⭐" },
    { rank: 7, name: "Rachel P.", points: 1620, reportsSubmitted: 29, badge: "⭐" },
    { rank: 8, name: "David S.", points: 1580, reportsSubmitted: 28, badge: "⭐" },
    { rank: 9, name: "Anna B.", points: 1490, reportsSubmitted: 26, badge: "⭐" },
    { rank: 10, name: "Chris H.", points: 1460, reportsSubmitted: 25, badge: "⭐" },
    { rank: 11, name: "Maria G.", points: 1440, reportsSubmitted: 24, badge: "⭐" },
    { rank: 12, name: "John D.", points: 1420, reportsSubmitted: 23, badge: "⭐" }
  ];
  ...
  const getBadgeForLevel = (level: number) => {
    if (level >= 20) return { name: "Civic Champion", color: "bg-purple-500", icon: "👑" };
    if (level >= 15) return { name: "Community Hero", color: "bg-gold-500", icon: "🏆" };
    if (level >= 10) return { name: "Active Citizen", color: "bg-blue-500", icon: "⭐" };
    if (level >= 5) return { name: "Helpful Reporter", color: "bg-green-500", icon: "📝" };
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span>{currentUser.points} / {currentUser.nextLevelPoints}</span>
            </div>
            <Progress 
            ...
            <div className="grid grid-cols-4 gap-4 mt-4">
              <div className="text-center">
                <div className="font-bold text-xl">{currentUser.reportsSubmitted}</div>
                <div className="text-xs text-blue-100">Reports</div>
              </div>
              <div className="text-center">
                <div className="font-bold text-xl">{currentUser.issuesResolved}</div>
            ...
            {challenges.map((challenge) => (
              <div key={challenge.id} className="border rounded-lg p-4 space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-semibold text-civic-blue-900">{challenge.title}</h3>
                    <p className="text-sm text-gray-600">{challenge.description}</p>
                  </div>
                  <Badge className={`${
                    challenge.type === 'weekly' ? 'bg-green-500' :
                    challenge.type === 'monthly' ? 'bg-blue-500' : 'bg-purple-500'
                  } text-white`}>
                    {challenge.type}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {challenge.progress === challenge.maxProgress && (
                    <Button size="sm" className="bg-civic-green-500 hover:bg-civic-green-600">
                      Claim Reward
                    </Button>
                  )}
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <Badge className={`${achievement.unlocked ? 'bg-green-500' : 'bg-gray-400'} text-white`}>
            ...
            {leaderboard.map((entry) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className={`font-medium ${entry.name === currentUser.name ? 'text-civic-blue-900' : ''}`}>
                      {entry.name} {entry.name === currentUser.name && '(You)'}
                    </div>
                    <div className="text-sm text-gray-600">{entry.reportsSubmitted} reports submitted</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-civic-blue-600">{entry.points}</div>
