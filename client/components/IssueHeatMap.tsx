...
export default function IssueHeatMap() {
  ...
  const [selectedHotspot, setSelectedHotspot] = useState<IssueHotspot | null>(null);
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <CardContent>
            ...
            <div className="relative bg-gradient-to-br from-blue-100 to-green-100 rounded-lg h-96 p-4 overflow-hidden">
              ...
              {filteredHotspots.map((hotspot, index) => (
                ...
                >
                  ...
                  {hotspot.status === 'active' && (
                    <div className={`absolute inset-0 ${getPriorityColor(hotspot.priority)} rounded-full animate-ping opacity-75`}></div>
                  )}
                </div>
              ))}
              ...
              <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm rounded-lg p-3 space-y-2">
                <div className="text-xs font-semibold text-gray-700">Issue Priority</div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {selectedHotspot ? "Detailed information about selected area" : "Top issue concentration areas"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {selectedHotspot ? (
              <div className="space-y-4">
                <div>
                  <h3 className="font-semibold text-lg text-civic-blue-900">{selectedHotspot.location}</h3>
                  <p className="text-sm text-gray-600 flex items-center gap-1">
                    <MapPin className="w-3 h-3" />
                    {selectedHotspot.coordinates[0]}, {selectedHotspot.coordinates[1]}
                  </p>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <div className="text-2xl font-bold text-civic-blue-600">{selectedHotspot.issueCount}</div>
                    <div className="text-xs text-gray-600">Total Issues</div>
                  </div>
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <Badge className={`${getPriorityColor(selectedHotspot.priority)} text-white`}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`flex items-center gap-1 ${getStatusColor(selectedHotspot.status)}`}>
                      {getStatusIcon(selectedHotspot.status)}
                      <span className="capitalize text-sm">{selectedHotspot.status.replace('_', ' ')}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="capitalize text-sm">{selectedHotspot.trend}</span>
                    </div>
                  </div>
                ...
                {filteredHotspots.slice(0, 5).map((hotspot) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 ${getPriorityColor(hotspot.priority)} rounded-full flex items-center justify-center text-white font-bold text-xs`}>
                        {hotspot.issueCount}
                      </div>
                      <div>
                        <div className="font-medium text-sm">{hotspot.location}</div>
                        <div className="text-xs text-gray-600 capitalize">{hotspot.category}</div>
                      </div>
                    </div>
        ...
        <Card>
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-bold text-red-600 mb-2">
              {filteredHotspots.filter(h => h.priority === 'urgent' || h.priority === 'high').length}
            </div>
